(ns backend.services.crusade
  (:require [backend.repositories.crusade :as repo]
            ...
            [backend.mappers.crusade :as mapper]
...
(defn update [tx id upsert-dto]
  ...
  (if (repo/exists-by-id tx id)
    (let [entity (assoc (mapper/to-entity upsert-dto) :crusade-id id)
...
(defn delete [tx id]
  (if (repo/exists-by-id tx id)
    (try
      ...
      true
      (catch Exception _ (throw (ex-info "Cannot delete Crusade because it is referenced by other records" {}))))
    false))