...
(defn get-all-handler [req]
  (let [tx (:tx req)]
    (api-response true 200 "All parishes fetched" (svc/get-all tx))))
...
(defn get-by-id-handler [req]
  (let [tx (:tx req)
    ...
    (if entity
      (api-response true 200 "Parish fetched successfully" entity)
      (api-response false 404 "Parish not found" nil))))

(defn create-handler [req]
  (let [tx (:tx req)
        body (json/parse-string (:body req) true)
        saved (svc/create tx body)]
    (api-response true 201 "Parish created successfully" saved)))
...
(defn update-handler [req]
  (let [tx (:tx req)
    ...
    (if updated
      (api-response true 200 "Parish updated successfully" updated)
      (api-response false 404 "Parish not found" nil))))
...
(defn delete-handler [req]
  (let [tx (:tx req)
    ...
    (if deleted
      (api-response true 200 "Parish deleted successfully" nil)
      (api-response false 404 "Parish not found" nil))))
