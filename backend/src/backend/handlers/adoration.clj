...
(defn get-all-handler [req]
  (let [tx (:tx req)
    ...
    (util/api-response true 200 "All adorations fetched" (vec filtered))))
...
(defn get-by-id-handler [req]
  (let [tx (:tx req)
    ...
    (if entity
      (util/api-response true 200 "Adoration fetched successfully" entity)
      (util/api-response false 404 "Adoration not found" nil))))

(defn create-handler [req]
  (let [tx (:tx req)
        body (json/parse-string (:body req) true)
        saved (svc/create tx body)]
    (util/api-response true 201 "Adoration created successfully" saved)))

(defn update-handler [req]
  (let [tx (:tx req)
        id (parse-long (get-in req [:path-params :id]))
        body (json/parse-string (:body req) true)
        updated (svc/update-adoration! tx id body)]
    (if updated
      (util/api-response true 200 "Adoration updated successfully" updated)
      (util/api-response false 404 "Adoration not found" nil))))
...
(defn delete-handler [req]
  (let [tx (:tx req)
    ...
    (if deleted
      (util/api-response true 200 "Adoration deleted successfully" nil)
      (util/api-response false 404 "Adoration not found" nil))))
